...
class Data_holder:
    ...
    def get_glove(self, word):
        ...
        if word == self.dictionary[index]:
            ...
            for i in range(self.Word_Embedding_Dimension):
                none_result[i] = 1.0 / self.Word_Embedding_Dimension
            return none_result
    ...
    def get_next_batch(self):
        np_range = numpy.arange(self.Batch_Index * 5000, self.Batch_Index * 5000 + 5000)
        ...
        if self.Paragraphs_Length[self.argsort_length[self.Batch_Index]] > 350:
            increase_step = 3

        while True:
            if cur_batch + self.Batch_Index > self.Total_Batch_Size - 2:
                cur_batch = self.Total_Batch_Size - self.Batch_Index
                break

            size = cur_batch * self.Paragraphs_Length[self.argsort_length[self.Batch_Index * 5000 + 5000]]
            if size < 400 * 100:
                cur_batch += increase_step
            else:
                break

        size_shape[0] = self.Total_Batch_Size - self.Batch_Index * self.Batch_Size

        p_max = self.Paragraphs_Length[self.argsort_length[self.Batch_Index * 5000 + 5000]]
        q_max = 0
        for i in range(cur_batch):
            idx = np_range[i]

            if q_max < self.Questions_Length[self.argsort_length[idx]]:
                q_max = self.Questions_Length[self.argsort_length[idx]]
        ...
        batch_start_index = numpy.zeros((cur_batch, p_max), dtype='f')
        batch_stop_index = numpy.zeros((cur_batch, p_max), dtype='f')
        ...
        batch_length = numpy.zeros((cur_batch), dtype=numpy.int32)
        batch_paragraph_str = []
        batch_question_str = []
        index = 0
        for i in range(cur_batch):
            ...
            batch_paragraph_str.append(self.Paragraphs[self.argsort_length[idx]])
            batch_question_str.append(self.Questions[self.argsort_length[idx]])

            batch_paragraph[index] = self.get_glove_sequence(length=p_max, tokens=self.Paragraphs[self.argsort_length[idx]])
            batch_question[index] = self.get_glove_sequence(length=q_max, tokens=self.Questions[self.argsort_length[idx]])

            batch_start_index[index, int(self.start_index_batch[self.argsort_length[idx]])] = 1
            batch_stop_index[index, int(self.stop_index_batch[self.argsort_length[idx]])] = 1
            batch_start_index_value[index] = int(self.start_index_batch[self.argsort_length[idx]])
            batch_stop_index_value[index] = int(self.stop_index_batch[self.argsort_length[idx]])
            batch_IDs[index] = str(self.IDs[self.argsort_length[idx]])
            batch_length[index] = int(self.Paragraphs_Length[self.argsort_length[idx]])
            #테스트 하기 위한 paragraph, question raw 리턴 해보셈 임시로
            index += 1

        self.Batch_Index += 1
        if self.Batch_Index == 17:
            self.Batch_Index = 0

        print(batch_paragraph.shape)

        return batch_paragraph, batch_question, batch_start_index, batch_stop_index, batch_start_index_value, batch_stop_index_value, batch_length, \
               batch_paragraph_str, batch_question_str
