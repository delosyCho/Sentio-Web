...
class dataprocessor:
    def __init__(self):
        ...
        file_p = open('WIKIQA_paragraph', 'w', encoding='utf8')
        file_q = open('WIKIQA_question', 'w', encoding='utf8')
        file_l = open('WIKIQA_label', 'w', encoding='utf8')

        for i in range(self.Batch_Index):
            for j in range(len(self.Paragraphs[i])):
                file_p.write(self.Paragraphs[i][j])
                if j != len(self.Paragraphs[i]) - 1:
                    file_p.write('\t')

            for j in range(len(self.Questions[i])):
                file_q.write(self.Questions[i][j])
                if j != len(self.Questions[i]) - 1:
                    file_q.write('\t')

            file_p.write('\a')
            file_q.write('\a')

            file_l.write(str(self.Labels[i]))
            file_l.write('\a')
