...
class Web_Crawler:
        ...
        for id_ in self.ID_list:
            ...
            for T in str_TK:
                if self.check_hangeul(T) is True:
                    ...
                    # input()
                    try:
                        searched_index = refined_text_.index()
                    except:
                        searched_index = -1

                    if searched_index == -1:
                        refined_text_.append(T)
                else:
                    0
                    # print('!!', T)
                    # input()

            point = np.zeros(shape=[len(str_TK)], dtype=np.float32)

            for T in refined_text:
                for idx, K in enumerate(str_TK):
                    if T == K:
                        score += 1.0
                        point[idx] = 1 * 1.5

            score = 0
            if len(refined_text) > 0 and point.sum() > 0:
                if len(refined_text) > len(refined_text_):
                    score = point.sum() / len(refined_text)
                #score *= point.sum()
            ...
            print(id_, ':', score, point.sum(), len(refined_text), len(refined_text_))
    ...
    def get_score_by_class(self, url, string):
        ...
        for id_ in self.Class_list:
            ...
            print(id_, ':', score, point.sum(), len(refined_text))
